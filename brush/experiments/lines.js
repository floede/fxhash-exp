function buildLinesIncrement(incrementX, incrementY) {
  ...
  const angle = () =>
    random(angleMiddle - PI * parabola, angleMiddle + PI * parabola);
  const shapes = [
...
function angledLineShape(col, start, end, angle) {
  ...
  const brush = compose(new Brush(col, 60), drawFixedAngled(angle, length));
...
function backgroundShape(col) {
  ...
  const brush = compose(new Brush(col, 60), drawFixedAngled(HALF_PI, length));
  const shape = compose(
    new Shape(brush),
