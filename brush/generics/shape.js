...
const lineShape = (x1, y1, x2, y2) => ({
  pick: () => {
    const ratio = random();
    return createVector(lerp(x1, x2, ratio), lerp(y1, y2, ratio));
  },
});
...
const maxFilledRatio = (ratio) => (self) => ({
  area: () => {
    if (self.computedArea) return self.computedArea;
    ...
    for (let i = 0; i < nPoints; i++) {
      ...
      if (self.isValidPoint(fakePoint)) validPoints++;
    }
...
const rectShape = (x, y, w, h) => ({
  pick: () => {
    return createVector(lerp(x, x + w, random()), lerp(y, y + h, random()));
  },
  isValidPoint: (point) => {
