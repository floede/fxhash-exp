const randomChooser = (values, weights) =>
  function () {
    if (weights === undefined) return random(values);
...
function compose(baseObject, ...objectBehaviors) {
  ...
  for (let behaviour of objectBehaviors) {
    try {
      composedObject = Object.assign(composedObject, behaviour(composedObject));
    } catch (TypeError) {
      composedObject = Object.assign(composedObject, behaviour);
    }
  }
